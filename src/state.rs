...
impl App {
    pub fn new(root: PathBuf, max_steps: usize, dup_backend: Backend) -> Result<Self> {
        Ok(Self {
            root,
    ...
    /// 按 agent 名取客户端（每个 agent 的配置在加载时已按环境变量解析）。
...
mod tests {
    ...
    fn app_contest_dir_roundtrip() {
        let app = App::new(PathBuf::from("/tmp"), 10, Backend::Cpret).unwrap();
