...
## 注意事项
TopCoder 上的题目都是实现函数的形式，这不符合一般 OI 题的习惯。\
当你搬运 TopCoder 题目时，**务必**将其改编为传统 OI 题的形式，通过标准输入输出流进行输入输出。\
很多 TopCoder 题目会传入一个 vector<string> 要求用户将字符串拼接后解析成整数等其他类型的原始数据，\
这种题目**务必**改编成直接以空格/换行分割的原始数据，便于读取。";
